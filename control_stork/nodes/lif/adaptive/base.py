...
class AdaptiveLIFGroup(CellGroup):
    def __init__(
        ...
        **kwargs
        ...
        super(AdaptiveLIFGroup, self).__init__(
            shape,
            dropout_p=dropout_p,
            stateful=stateful,
            name=name,
        ...
        self.tau_mem_init = (
            torch.Tensor([tau_mem]) if not torch.is_tensor(tau_mem) else tau_mem
        )
        self.tau_syn_init = (
            torch.Tensor([tau_syn]) if not torch.is_tensor(tau_syn) else tau_syn
        )
        self.tau_ada_init = (
            torch.Tensor([tau_ada]) if not torch.is_tensor(tau_ada) else tau_ada
        )
        self.threshold = threshold
        self.threshold_decay = (
            torch.Tensor([threshold_decay])
            if not torch.is_tensor(threshold_decay)
            else threshold_decay
        )
        assert torch.all(
            self.threshold_decay >= 0.0
        ), "threshold_decay must be non-negative"
        self.threshold_xi = (
            torch.Tensor([threshold_xi])
            if not torch.is_tensor(threshold_xi)
            else threshold_xi
        )
        ...
        assert self.mem_param in [
            "full",
            "single",
        ], "mem_param must be either 'full' or 'single'"
        if self.mem_param == "single":
            assert self.tau_mem_init.shape == (
                1,
            ), "tau_mem must be a scalar or a tensor of shape (1,) if mem_param is 'single'"
        self.syn_param = syn_param.lower()
        assert self.syn_param in [
            "full",
            "single",
        ], "syn_param must be either 'full' or 'single'"
        if self.syn_param == "single":
            assert self.tau_syn_init.shape == (
                1,
            ), "tau_syn must be a scalar or a tensor of shape (1,) if syn_param is 'single'"
        self.ada_param = ada_param.lower()
        assert self.ada_param in [
            "full",
            "single",
        ], "ada_param must be either 'full' or 'single'"
        if self.ada_param == "single":
            assert self.tau_ada_init.shape == (
                1,
            ), "tau_ada must be a scalar or a tensor of shape (1,) if ada_param is 'single'"
        self.reset = reset.lower()
        assert self.reset in ["sub", "set"], "reset must be either 'sub' or 'set'"
        self.reset_mem = (
            self.subtractive_reset if self.reset == "sub" else self.multiplicative_reset
        )
    ...
    def configure(self, time_step: float, device: torch.device, dtype):
        ...
        if self.learn_param == "tau":
            self._tau_mem = torch.nn.Parameter(
                torch.ones(self.shape if self.mem_param == "full" else 1, device=device)
                * self.tau_mem_init.to(device),
                requires_grad=self.learn_mem,
            )
            self.constraints[self._tau_mem] = (1e-3, torch.inf)
            self._tau_syn = torch.nn.Parameter(
                torch.ones(self.shape if self.syn_param == "full" else 1, device=device)
                * self.tau_syn_init.to(device),
                requires_grad=self.learn_syn,
            )
            self.constraints[self._tau_syn] = (1e-3, torch.inf)
            self._tau_ada = torch.nn.Parameter(
                torch.ones(self.shape if self.ada_param == "full" else 1, device=device)
                * self.tau_ada_init.to(device),
            ...
            self._beta_mem = torch.nn.Parameter(
                self.tau_to_beta(
                    torch.ones(
                        self.shape if self.mem_param == "full" else 1, device=device
                    )
                    * self.tau_mem_init
                ),
                requires_grad=self.learn_mem,
            ...
            self._beta_syn = torch.nn.Parameter(
                self.tau_to_beta(
                    torch.ones(
                        self.shape if self.syn_param == "full" else 1, device=device
                    )
                    * self.tau_syn_init
                ),
                requires_grad=self.learn_syn,
            ...
            self._beta_ada = torch.nn.Parameter(
                self.tau_to_beta(
                    torch.ones(
                        self.shape if self.ada_param == "full" else 1, device=device
                    )
                    * self.tau_ada_init
                ),
                requires_grad=self.learn_ada,
    ...
    def multiplicative_reset(
        self, mem: torch.Tensor, rst: torch.Tensor
    ) -> torch.Tensor:
        return mem * (1.0 - rst)
    ...
    def forward(self) -> None:
        ...
        self.mem = self.states["mem"] = (
            self.mem * self._beta_mem
            + self.syn * (1.0 - self._beta_mem)
            - self.rst * self.vt
        )
        ...
        self.nt = self.states["nt"] = (
            self.nt * self._beta_ada + (1.0 - self._beta_ada) * self.rst
        )
        self.vt = self.states["vt"] = self.bt + self.threshold_xi * self.nt
